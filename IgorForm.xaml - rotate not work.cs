...
{
    /// <summary>
    /// Логика взаимодействия для IgorForm.xaml
    /// </summary>
    public partial class IgorshemForm : Form
    {
        ...
        Project pProject;
        //[bWithWindow] = если установлено значение true\: открывает дополнительное диалоговое окно выбора, если в браузере страниц было выбрано более одного проекта.
        private Page CreateNewPage(string newPageName, bool bWithWindow=true)
        {
            ...
            string input = Interaction.InputBox("В проект [" + pProject.ProjectName + "]",//Prompt
                       "Добавляем лист:",//Title
                       newPageName);//DefaultResponse
        ...
        /*
        private string PropsToStr(object obj)
        {
            ...
            foreach (var prop in props)
            {
                if (prop.GetIndexParameters().Length == 0)
                    ...
                    str += "[" + j + "-th] "
                        + prop.Name
                        + " (" + prop.PropertyType.Name + "): <" + prop.GetIndexParameters().Length + ">\n";
                //                str += "  {\n" + PropsToStr(prop) + "}\n";//рекурсия вызывает зависание
                j++;
            }
            return str;
        }*/
        /*private string Article_Example1(Project oProject)
        {
            string str = "";
            foreach (Article oArticle in oProject.Articles)
            {
                ...
                if (!oArticle.Properties.ARTICLE_DEPTH.IsEmpty)
                    str += "ARTICLE_DEPTH : " + oArticle.Properties.ARTICLE_DEPTH + "\n";
                if (!oArticle.Properties.ARTICLE_VARIANT.IsEmpty)
                    str += "ARTICLE_VARIANT : " + oArticle.Properties.ARTICLE_VARIANT + "\n";

                foreach (int nIndex in oArticle.Properties.ARTICLE_FREE_DATA_DESCRIPTION.Indexes)
                    str += "ARTICLE_FREE_DATA_DESCRIPTION[" + nIndex + "] : " + oArticle.Properties.ARTICLE_FREE_DATA_DESCRIPTION[nIndex] + "\n";

                foreach (int nIndex in oArticle.Properties.ARTICLE_FREE_DATA_UNIT.Indexes)
                    str += "ARTICLE_FREE_DATA_UNIT[" + nIndex + "] : " + oArticle.Properties.ARTICLE_FREE_DATA_UNIT[nIndex] + "\n";

                foreach (int nIndex in oArticle.Properties.ARTICLE_FREE_DATA_VALUE.Indexes)
                    str += "ARTICLE_FREE_DATA_VALUE[" + nIndex + "] :" + oArticle.Properties.ARTICLE_FREE_DATA_VALUE[nIndex] + "\n";
                //                }
            }
            return str;
        }*/
        ...
        {
            //System.Windows.Forms.MessageBox.Show("DefenceType = <" + this.ComboDefenceType.Text + ">");
            if (this.ComboDefenceType.Text == "Автоматический выключатель") {
                try {
                    ...
                    PageAssignFunction(pProject, oPage, "IEC_symbol", "QL3", 1, "-Q1", ConnDesignation, Coords, 90);
                } catch {
                    return;
                }
            } else {//"Предохранители"
                try
                {
                    ...
                    ConnDesignation[5] = "6";
                    Point Coords = new Point(60, 250);
                    PageAssignFunction(pProject, oPage, "IEC_symbol", "FLTR3", 1, "-F1", ConnDesignation, Coords, 1.5708);
                }
                catch
                {
                    return;
        ...
        private void PageAssignFunction(Project pProject, Page oPage, string LibName, string SymbName, int nVar, 
            string FuncName, string[] ConnDesignation, System.Windows.Point Coords, double angle=0)
        {
            try
            {
                //Get SymbolVariant
                ...
                if (angle != 0)
                    FunctionRotate(oSymbolVariant.SubPlacements, oNewFunction.Location, angle);
            }
            catch
            {
                return;
        ...
        private void FunctionRotate(Placement[] plcmnts, EplApi.Base.PointD oPntStart, double angle)
        {
            Edit myEd = new Edit();
            myEd.Rotate(plcmnts, oPntStart, angle);
            bool redr = myEd.RedrawGed();
            System.Windows.Forms.MessageBox.Show("redr = [" + redr.ToString() + "]");
        }
    }
}
